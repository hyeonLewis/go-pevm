...
func NewAccessListTracer(state *state.StateDB, multiVersionStore MultiVersionStore, transactionIndex int, incarnation int, abortedChannel chan Abort) *AccessListTracer {
...
func (a *AccessListTracer) CaptureState(env *vm.EVM, pc uint64, op vm.OpCode, gas, cost, ccLeft, ccOpcode uint64, scope *vm.ScopeContext, depth int, err error) {
	...
	case vm.SLOAD:
		if stackLen >= 1 {
			slot := common.Hash(stackData[stackLen-1].Bytes32())
			value := a.state.GetState(scope.Contract.Address(), slot)
			storageKey := ToStorageKey(scope.Contract.Address(), slot)
	...
	case vm.EXTCODECOPY:
		if stackLen >= 4 {
			...
			var (
				codeOffset = stackData[stackLen-3]
				length     = stackData[stackLen-4]
			)
			uint64CodeOffset, overflow := codeOffset.Uint64WithOverflow()
			if overflow {
				uint64CodeOffset = 0xffffffffffffffff
			}
			codeCopy := getData(a.state.GetCode(addr), uint64CodeOffset, length.Uint64())
			storageKey := ToStorageKey(addr, CodeCopyKey)
			a.ValidGet(storageKey)
			if _, ok := a.excl[addr]; !ok {
				a.UpdateReadSet(storageKey, common.BytesToHash(codeCopy))
			}
		}
	case vm.EXTCODEHASH:
		if stackLen >= 1 {
			addr := common.Address(stackData[stackLen-1].Bytes20())
			codeHash := a.state.GetCodeHash(addr)
			if _, ok := a.excl[addr]; !ok {
	...
	case vm.EXTCODESIZE:
		if stackLen >= 1 {
			addr := common.Address(stackData[stackLen-1].Bytes20())
			codeSize := a.state.GetCodeSize(addr)
			if _, ok := a.excl[addr]; !ok {
	...
	case vm.BALANCE:
		if stackLen >= 1 {
			addr := common.Address(stackData[stackLen-1].Bytes20())
			balance := a.state.GetBalance(addr)
			if _, ok := a.excl[addr]; !ok {
