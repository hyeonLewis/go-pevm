...
func TestMultiversionItemGetLatest(t *testing.T) {
	...
	one := common.BytesToHash([]byte("one"))
	mvItem.Set(1, 0, one)
	value, found = mvItem.GetLatest()
	...
	zero := common.BytesToHash([]byte("zero"))
	mvItem.Set(0, 0, zero)
	value, found = mvItem.GetLatest()
	require.True(t, found)
	require.Equal(t, one, value.Value())
	...
	require.Equal(t, common.Hash{}, value.Value())

	// Overwrite the deleted value with some data
	two := common.BytesToHash([]byte("two"))
	mvItem.Set(2, 3, two)
	value, found = mvItem.GetLatest()
	require.True(t, found)
...
func TestMultiversionItemGetByIndex(t *testing.T) {
	...
	four := common.BytesToHash([]byte("four"))
	mvItem.Set(4, 0, four)
	// also reads the four
...
func TestMultiversionItemRemove(t *testing.T) {
	...
	mvItem.Set(1, 0, common.BytesToHash([]byte("one")))
	mvItem.Set(2, 0, common.BytesToHash([]byte("two")))

	mvItem.Remove(2)
	value, found := mvItem.GetLatest()
	require.True(t, found)
	require.Equal(t, common.BytesToHash([]byte("one")), value.Value())
}
